        ...
        <?php
            ...
            if (!empty($_POST['answer'])){
                ...
                for ($i=0; $i < $chosenone; $i++){
                    ...
                    //$zapytanie = 'select * from collection where category = "' . $categoryTable . '" AND where id=' . $wylosowane[$total];
                    $zapytanie = "SELECT * FROM collection WHERE category='" . $categoryTable . "' AND id=" . $wylosowane[$total];
    ...
    <script src="scriptquiz.js"></script>
        <?php 
            ...
            for ($i=1; $i <= $chosenone; $i++){
                $zapytanie = "SELECT * FROM collection WHERE category='" . $categoryTable . "' AND id=" . $wylosowane[$radio_name];
                $wynik = mysqli_query($polaczenie, $zapytanie);
                ...
                $sql = "SELECT image, type FROM images where id_number = " . $wylosowane[$radio_name];
                $res = mysqli_query($polaczenie, $sql);
                $rowi = mysqli_fetch_assoc($res);
                if (!empty($rowi['image'])){
                    echo "<img src='data:" . $rowi["type"] . ";base64," . base64_encode($rowi["image"]) . "'>";
                };
